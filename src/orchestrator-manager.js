...
function createOrchestration(opts = {}) {
    ...
    // Check concurrent limit — only count sessions actively orchestrating
    const activeCount = Array.from(orchestrations.values()).filter(
        s => s._activeOrchestration
    ).length;
    if (activeCount >= cfg.maxConcurrentOrchestrations) {
...
function createChatSession({ workspace, config } = {}) {
    const cfg = _getConfig();
    const id = crypto.randomUUID();
    const session = new OrchestratorSession(id, {
        workspace,
        lsInst: resolveLsInst(workspace),
        ...cfg,
        ...(config || {}),
    });

    // Register but do NOT call start() — chat() will handle planner init lazily
    orchestrations.set(id, session);

    session.on('destroyed', () => {
        _archiveToHistory(id, session);
        orchestrations.delete(id);
        _broadcast('orchestration_destroyed', { orchestrationId: id });
    });
    ...
    for (const evt of ['orch_started', 'orch_analysis', 'orch_plan', 'orch_awaiting_approval',
        'orch_executing', 'orch_subtask_update', 'orch_phase_complete', 'orch_clarification',
        'orch_review', 'orch_completed', 'orch_failed', 'orch_cancelled', 'log', 'state_change']) {
        session.on(evt, (data) => _broadcast(evt, { orchestrationId: id, ...data }));
    }

    _broadcast('chat_session_created', { orchestrationId: id, workspace });
    return session;
}
...
module.exports = {
    createOrchestration, createChatSession, getOrchestration, destroyOrchestration,
